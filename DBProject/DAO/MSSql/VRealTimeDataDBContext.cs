...
//        }
        ...
        {
            string tablename = GetTableName(pid);// "t_SM_ReallTimeData_" + pid.ToString("00000");
            string query = $"select a.TagID, d.Name DeviceTypeName, DeviceName, f.Units Units, b.OrderBy" +
                            $" from (select TagID, PID, DID, CID, DataTypeID" +
                            $"    from t_CM_PointsInfo" +
                            $"    where DataTypeID!=23 and DataTypeID!=82 and PID={pid}) a left join t_DM_DeviceInfo b on a.DID=b.DID and a.PID=b.PID left join t_CM_DeviceType d on b.DTID=d." +
                            $"DTID left join t_CM_ValueType f on a.DataTypeID=f.DataTypeID left join t_DM_CircuitInfo e on a.CID= e.CID and a.PID=e.PID";
            return SQLQuery<t_V_RealTimeData>(query);
        }
