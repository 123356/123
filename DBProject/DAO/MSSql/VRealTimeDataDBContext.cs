...
namespace DAO
{
    public class VRealTimeDataDBContext:DBContextBase,IVRealTimeData
    {
        ...
        public IList<t_V_RealTimeTab> GetTagIdDetail(int pageSize, int nPage, int pid, int cid, int tdid, int did)
        {
            ...
            string query = string.Format("select top {0} a.TagID, d.Name DeviceTypeName, DeviceName,中文描述 Position,f.Units Units,CName,b.OrderBy from "
...
+ " where a.RowNumber>{2} {3} {4} {5}"
+ " order by OrderBy", pageSize, condition1, pageSize * (nPage - 1), condition2, condition3, condition4);
            return SQLQuery<t_V_RealTimeTab>(query);
        }
        ...
        public IList<t_V_RealTimePV> GetTagIdPV(int pid, string tages)
        {
            ...
            string query = $"SELECT * FROM {tablename} WHERE TagID in({tages})";
            return SQLQuery<t_V_RealTimePV>(query);
        }
